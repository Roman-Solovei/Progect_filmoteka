...
export function renderModalFilm() {
    mainContainer.galleryContainer.addEventListener('click', (event) => {
        ...
        apiService.fetchFilm(targetFilm.id).then(film => {     
            const modalGenres = film.genres.flatMap((genre) => genre.name).slice(0, 2);
            const logo = film.production_companies.flatMap((logo) => logo.logo_path).slice(0, 1);           
            film.genres_list = modalGenres;             
            film.img1x = createURLImg(film.poster_path, 1);
            film.img2x = createURLImg(film.poster_path, 2);   
            film.logo = logo;
            const markup = modalFilm(film);
